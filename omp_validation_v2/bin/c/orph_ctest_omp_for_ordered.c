...
int orph_ctest_omp_for_ordered (FILE * logFile)
{
...
int main()
{
	...
	static const char * logFileName = "bin/c/orph_ctest_omp_for_ordered.log";	/* name of the logfile */
	...
	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of orph_ctest_omp_for_ordered out of %d\n\n",i+1,REPETITIONS);
		if(orph_ctest_omp_for_ordered(logFile)){
...
void orph1_omp_for_ordered (FILE * logFile) {
	    int i;
	    int my_islarger = 1;
#pragma omp for schedule(static,1) ordered
	    for (i = 1; i < 100; i++)
