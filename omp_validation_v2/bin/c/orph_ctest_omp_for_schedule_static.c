...
int orph_ctest_omp_for_schedule_static (FILE * logFile)
{
  int threads;
  int i,lasttid;
...
int main()
{
	...
	static const char * logFileName = "bin/c/orph_ctest_omp_for_schedule_static.log";	/* name of the logfile */
	...
	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of orph_ctest_omp_for_schedule_static out of %d\n\n",i+1,REPETITIONS);
		if(orph_ctest_omp_for_schedule_static(logFile)){
...
void orph1_omp_for_schedule_static (FILE * logFile) {
	double count;
      int tid;
      int j;
