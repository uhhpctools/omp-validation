...
int orph_ctest_omp_task(FILE * logFile){
...
int main()
{
	...
	static const char * logFileName = "bin/c/orph_ctest_omp_task.log";	/* name of the logfile */
	...
	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of orph_ctest_omp_task out of %d\n\n",i+1,REPETITIONS);
		if(orph_ctest_omp_task(logFile)){
...
void orph1_omp_task (FILE * logFile) {
            /* First we have to store the value of the loop index in a new variable
             * which will be private for each task because otherwise it will be overwritten
             * if the execution of the task takes longer than the time which is needed to 
             * enter the next step of the loop!
