...
int test_omp_parallel_reduction(FILE * logFile){
	int sum=0;
	int known_sum;
	double dsum=0;
	double dknown_sum;
...
int main()
{
	...
	static const char * logFileName = "bin/c/test_omp_parallel_reduction.log";	/* name of the logfile */
	...
	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of test_omp_parallel_reduction out of %d\n\n",i+1,REPETITIONS);
		if(test_omp_parallel_reduction(logFile)){
