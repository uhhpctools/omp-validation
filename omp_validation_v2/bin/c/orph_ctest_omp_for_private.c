...
int orph_ctest_omp_for_private (FILE * logFile)
{
    int sum = 0;
...
int main()
{
	...
	static const char * logFileName = "bin/c/orph_ctest_omp_for_private.log";	/* name of the logfile */
	...
	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of orph_ctest_omp_for_private out of %d\n\n",i+1,REPETITIONS);
		if(orph_ctest_omp_for_private(logFile)){
...
void orph1_omp_for_private (FILE * logFile) {
	    int i;
#pragma omp for  schedule(static,1)
	    for (i = 1; i <= LOOPCOUNT; i++)
	    {
