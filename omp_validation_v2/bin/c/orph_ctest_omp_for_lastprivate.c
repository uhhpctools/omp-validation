...
int orph_ctest_omp_for_lastprivate (FILE * logFile)
{
	int sum = 0;
	int known_sum;
...
int main()
{
	...
	static const char * logFileName = "bin/c/orph_ctest_omp_for_lastprivate.log";	/* name of the logfile */
	...
	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of orph_ctest_omp_for_lastprivate out of %d\n\n",i+1,REPETITIONS);
		if(orph_ctest_omp_for_lastprivate(logFile)){
...
void orph1_omp_for_lastprivate (FILE * logFile) {
		int i;
#pragma omp for schedule(static,7) 
		for (i = 1; i <= LOOPCOUNT; i++)
		{
