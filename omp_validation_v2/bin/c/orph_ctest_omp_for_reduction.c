...
int orph_ctest_omp_for_reduction (FILE * logFile)
{
...
int main()
{
	...
	static const char * logFileName = "bin/c/orph_ctest_omp_for_reduction.log";	/* name of the logfile */
	...
	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of orph_ctest_omp_for_reduction out of %d\n\n",i+1,REPETITIONS);
		if(orph_ctest_omp_for_reduction(logFile)){
...
void orph1_omp_for_reduction (FILE * logFile) {
	    int j;
#pragma omp for schedule(dynamic,1) 
	    for (j = 1; j <= LOOPCOUNT; j++)
	    {
...
void orph2_omp_for_reduction (FILE * logFile) {
	    int j;
#pragma omp for schedule(dynamic,1) 
	    for (j = 1; j <= LOOPCOUNT; j++)
	    {
...
void orph3_omp_for_reduction (FILE * logFile) {
	    int j;
#pragma omp for schedule(dynamic,1) 
	    for (j = 1; j <= MAX_FACTOR; j++)
	    {
...
void orph4_omp_for_reduction (FILE * logFile) {
	    int j;
#pragma omp for schedule(dynamic,1) 
	    for (j = 0; j < DOUBLE_DIGITS; j++)
	    {	
...
void orph5_omp_for_reduction (FILE * logFile) {
	    int j;
#pragma omp for schedule(dynamic,1) 
	    for (j = 0; j < DOUBLE_DIGITS; ++j)
	    {
...
void orph6_omp_for_reduction (FILE * logFile) {
	    int j;
#pragma omp for schedule(dynamic,1) 
	    for (j = 0; j < LOOPCOUNT; ++j)
	    {
...
void orph7_omp_for_reduction (FILE * logFile) {
	    int j;
#pragma omp for schedule(dynamic,1) 
	    for (j = 0; j < LOOPCOUNT; ++j)
	    {
...
void orph8_omp_for_reduction (FILE * logFile) {
	    int j;
#pragma omp for schedule(dynamic,1) 
	    for (j = 0; j < LOOPCOUNT; ++j)
	    {
...
void orph9_omp_for_reduction (FILE * logFile) {
	    int j;
#pragma omp for schedule(dynamic,1) 
	    for (j = 0; j < LOOPCOUNT; ++j)
	    {
...
void orph10_omp_for_reduction (FILE * logFile) {
	    int j;
#pragma omp for schedule(dynamic,1) 
	    for (j = 0; j < LOOPCOUNT; ++j)
	    {
...
void orph11_omp_for_reduction (FILE * logFile) {
	    int j;
#pragma omp for schedule(dynamic,1) 
	    for (j = 0; j < LOOPCOUNT; ++j)
	    {
...
void orph12_omp_for_reduction (FILE * logFile) {
	    int j;
#pragma omp for schedule(dynamic,1) 
	    for (j = 0; j < LOOPCOUNT; ++j)
	    {
...
void orph13_omp_for_reduction (FILE * logFile) {
	    int j;
#pragma omp for schedule(dynamic,1) 
	    for (j = 0; j < LOOPCOUNT; ++j)
	    {
...
void orph14_omp_for_reduction (FILE * logFile) {
	    int j;
#pragma omp for schedule(dynamic,1) 
	    for (j = 0; j < LOOPCOUNT; ++j)
	    {
...
void orph15_omp_for_reduction (FILE * logFile) {
	    int j;
#pragma omp for schedule(dynamic,1) 
	    for (j = 0; j < LOOPCOUNT; ++j)
	    {
