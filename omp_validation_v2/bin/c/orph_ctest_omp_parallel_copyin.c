...
int orph_ctest_omp_parallel_copyin(FILE * logFile)
{
...
int main()
{
	...
	static const char * logFileName = "bin/c/orph_ctest_omp_parallel_copyin.log";	/* name of the logfile */
	...
	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of orph_ctest_omp_parallel_copyin out of %d\n\n",i+1,REPETITIONS);
		if(orph_ctest_omp_parallel_copyin(logFile)){
...
void orph1_omp_parallel_copyin (FILE * logFile) {
	int i;
#pragma omp for 
	    for (i = 1; i < 1000; i++)
	    {
