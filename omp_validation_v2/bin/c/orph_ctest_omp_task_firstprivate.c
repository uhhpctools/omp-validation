...
int orph_ctest_omp_task_firstprivate (FILE * logFile)
{
    int i;
...
int main()
{
	...
	static const char * logFileName = "bin/c/orph_ctest_omp_task_firstprivate.log";	/* name of the logfile */
	...
	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of orph_ctest_omp_task_firstprivate out of %d\n\n",i+1,REPETITIONS);
		if(orph_ctest_omp_task_firstprivate(logFile)){
...
void orph1_omp_task_firstprivate (FILE * logFile) {
#pragma omp task 
                {
                    int j;
                    for (j = 0; j <= LOOPCOUNT; j++) {
