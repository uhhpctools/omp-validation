...
int ctest_omp_threadprivate(FILE * logFile)
{
	int sum = 0;
	int known_sum;
	int i; 
...
int main()
{
	...
	static const char * logFileName = "bin/c/ctest_omp_threadprivate.log";	/* name of the logfile */
	...
	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of ctest_omp_threadprivate out of %d\n\n",i+1,REPETITIONS);
		if(ctest_omp_threadprivate(logFile)){
