...
int orph_ctest_omp_for_schedule_static_3 (FILE * logFile)
{
  int threads;
  int i,lasttid;
...
int main()
{
	...
	static const char * logFileName = "bin/c/orph_ctest_omp_for_schedule_static_3.log";	/* name of the logfile */
	...
	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of orph_ctest_omp_for_schedule_static_3 out of %d\n\n",i+1,REPETITIONS);
		if(orph_ctest_omp_for_schedule_static_3(logFile)){
...
void orph1_omp_for_schedule_static_3 (FILE * logFile) {
	double count;
      int tid;
      int j;
...
void orph2_omp_for_schedule_static_3 (FILE * logFile) {
      {
          int n;
#pragma omp for  nowait
          for (n = 0; n < LOOPCOUNT; n++)
...
void orph3_omp_for_schedule_static_3 (FILE * logFile) {
      {
          int m;
#pragma omp for  nowait
          for (m = 1; m <= LOOPCOUNT; m++)
