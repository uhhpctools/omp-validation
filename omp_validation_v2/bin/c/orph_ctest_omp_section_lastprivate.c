...
int orph_ctest_omp_section_lastprivate(FILE * logFile){
...
int main()
{
	...
	static const char * logFileName = "bin/c/orph_ctest_omp_section_lastprivate.log";	/* name of the logfile */
	...
	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of orph_ctest_omp_section_lastprivate out of %d\n\n",i+1,REPETITIONS);
		if(orph_ctest_omp_section_lastprivate(logFile)){
...
void orph1_omp_section_lastprivate (FILE * logFile) {
#pragma omp sections private(i0) private(i,sum0)
	{
#pragma omp section  
	    {
