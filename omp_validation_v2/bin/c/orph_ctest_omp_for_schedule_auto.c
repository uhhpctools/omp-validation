...
int orph_ctest_omp_for_auto (FILE * logFile)
{
    int sum;
...
int main()
{
	...
	static const char * logFileName = "bin/c/orph_ctest_omp_for_auto.log";	/* name of the logfile */
	...
	fprintf(logFile,"##############################################\n");
	fprintf(logFile,"Testing omp for auto\n\n");

	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of orph_ctest_omp_for_auto out of %d\n\n",i+1,REPETITIONS);
		if(orph_ctest_omp_for_auto(logFile)){
...
void orph1_omp_for_auto (FILE * logFile) {
	int i;
#pragma omp for private(sum0)
	for (i = 1; i <= LOOPCOUNT; i++)
	{
