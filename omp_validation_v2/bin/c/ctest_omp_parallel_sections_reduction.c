...
int ctest_omp_parallel_sections_reduction(FILE * logFile){
	int sum=7;
	int known_sum;	
	double dpt=1,dsum=0;
	double dknown_sum;
...
int main()
{
	...
	static const char * logFileName = "bin/c/ctest_omp_parallel_sections_reduction.log";	/* name of the logfile */
	...
	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of ctest_omp_parallel_sections_reduction out of %d\n\n",i+1,REPETITIONS);
		if(ctest_omp_parallel_sections_reduction(logFile)){
