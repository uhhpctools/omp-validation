...
int orph_ctest_omp_parallel_firstprivate(FILE * logFile)
{
...
int main()
{
	...
	static const char * logFileName = "bin/c/orph_ctest_omp_parallel_firstprivate.log";	/* name of the logfile */
	...
	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of orph_ctest_omp_parallel_firstprivate out of %d\n\n",i+1,REPETITIONS);
		if(orph_ctest_omp_parallel_firstprivate(logFile)){
...
void orph1_omp_parallel_firstprivate (FILE * logFile) {
	int i;
#pragma omp for 
	    for (i = 1; i < 1000; i++)
	    {
