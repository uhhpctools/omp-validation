...
int orph_ctest_omp_barrier (FILE * logFile)
{
...
int main()
{
	...
	static const char * logFileName = "bin/c/orph_ctest_omp_barrier.log";	/* name of the logfile */
	...
	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (logFile, "\n\n%d. run of orph_ctest_omp_barrier out of %d\n\n",i+1,REPETITIONS);
		if(orph_ctest_omp_barrier(logFile)){
...
void orph1_omp_barrier (FILE * logFile) {
	int rank;
	rank = omp_get_thread_num ();
	if (rank == 1) {
	    my_sleep (SLEEPTIME);
